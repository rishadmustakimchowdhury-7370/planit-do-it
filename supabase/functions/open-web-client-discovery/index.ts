...
function buildPrompt(f: Filters) {
  ...
  const targetCompanies = mode === "strict" ? 25 : mode === "broad" ? 80 : 50;
  return `You are an experienced B2B Business Development Manager building a target account list.
...
TASK
1. Internally brainstorm 10–20 search strategies (alternative industry phrasings, sub-niches, adjacent terms).
   Example for "Recruitment Agencies UK": Recruitment Agency, Staffing Company, Executive Search Firm,
   Talent Solutions Company, Recruitment Consultancy, RPO Provider, Headhunting Firm, etc.
2. Using your public knowledge of real companies matching those strategies, return the top ${targetCompanies}
   REAL companies that match the brief. Do not invent companies. If unsure of a fact, leave it null.
3. For each company, list 2–5 likely decision makers by ROLE TITLE (CEO, Founder, Managing Director,
   Talent Acquisition Manager, HR Manager, Recruitment Manager, Head of Talent, Operations Director,
   etc.). Only include a real person's name + LinkedIn URL if you are confident it is correct;
   otherwise leave name and linkedin_url null and return just the title.
4. Score each company's fit:
   - "high"    = strong industry + size + location match, clear hiring/growth signals
   - "medium"  = partial match
   - "low"     = weak match but still relevant
5. Deduplicate by company name and website.
