...
export function SubmissionHistoryTable({
  ...
  async function load() {
    ...
    const [{ data: reports }, { data: profs }, { data: cand }, { data: job }] = await Promise.all([
      reportIds.length
        ? supabase.from("client_submission_reports").select("id, version").in("id", reportIds)
        : Promise.resolve({ data: [] as any[] }),
      recruiterIds.length
        ? supabase.from("profiles").select("user_id, full_name").in("user_id", recruiterIds)
        : Promise.resolve({ data: [] as any[] }),
